...
thetaInit = pi/4; % Theta initial pour le mode différentiel
...
preambule = repmat(preambule,1,2); % repmat() : répète le motif de la matrice préambule sur une fois en ligne et deux fois en colonne
Ep = sum(preambule.^2); %énergie de la séquence de préambule

%% Filtre adapté

g=rcosdesign(roll_off,2*M, Fse, 'sqrt');
ga = conj(fliplr(g)); % ga(t) = g(Ts-t) (le retard de Ts est implicite en numérique car on travaille avec des vecteurs causaux)

%% Acquisition des données du récepteur 
...
T = 273.15 + 25; % Température bruit du récepteurans
N0 = k*T;
varb = N0/2;
s=yl_Te;
tau0=1000*rand;
%tau0=1;
tau0=floor(tau0); %retard dû à la propagation
alpha0 = sqrt(1e-8);  % coefficient d'atténuation du canal
h_l = alpha0*[zeros(1,tau0) 1]; %dirac décalé de tau0*Te
Ps = 1/Te*mean(abs(s).^2);
yl_Te = sqrt(Ptx/Ps)*yl_Te/127;
y = conv(yl_Te,h_l); % signal en sortie du canal

%% Recepteur
Deltaf = 50e-6*f0*2*(rand-0.5); % erreur de fréquence OL précis à +-50ppm , 2*(rand-0.5)=nombre aléatoire dans l'intervalle [-1,1]
%Deltaf = 0;
...
%% Recepteur
yl_Te = yl_Te./norm(yl_Te); %% normalisation : utile ? => sûrement pour faire l'intercorrélation entre le préambule et le préambule reçu
test=yl_Te;

%% fonction d'interrcorr�lation
rho=filter(fliplr(preambule),1,abs(yl_Te))./(sqrt(Ep)*sqrt(...
    filter(ones(1,length(preambule)),1,abs(yl_Te).^2)));
figure, plot(rho),
title("fonction d'intercorr�lation")
xlabel('Nombre d''�chantillons')
ylabel ('?(u)')
%% Pr�-synchronisation temporelle 
rl_Ts_conc=0;
